...
async fn get_mypath(_req: Request<Body>) -> Result<Response<Body>, Infallible> {
    if _req.uri().path() != "/v1/avatar" || _req.method() != hyper::Method::GET {
    ...
    *response.status_mut() = StatusCode::OK;
    ...
    response.headers_mut().insert(hyper::header::LOCATION, hyper::header::HeaderValue::from_static("/v1/avatar"));
